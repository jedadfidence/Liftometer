...
type SortColumn =
  | "accountName" | "name" | "id" | "advertisingChannelType"
  | "status" | "budget" | "biddingStrategyType";

type SortDirection = "asc" | "desc";

const COLUMN_LABELS: Record<SortColumn, string> = {
  accountName: "Account", name: "Campaign Name", id: "Campaign ID",
  advertisingChannelType: "Type", status: "Status", budget: "Budget",
...
function getSortValue(campaign: GadsCampaign, column: SortColumn): string | number {
  switch (column) {
    case "budget": return campaign.budget.amountMicros;
    default: return campaign[column];
  }
}
...
export function CampaignTable({ campaigns }: CampaignTableProps) {
  ...
  const [filters, setFilters] = useState<Filters>({ account: [], status: [], type: [], bidding: [] });
  ...
  const filtered = useMemo(() => {
    ...
    return campaigns
      .filter((c) => {
        if (lowerSearch && !c.name.toLowerCase().includes(lowerSearch) && !c.id.toLowerCase().includes(lowerSearch)) return false;
  ...
  return (
    <div className="space-y-4">
      <div className="relative max-w-md">
        ...
        <Input placeholder="Search campaigns by name or ID..." value={rawSearch} onChange={(e) => setRawSearch(e.target.value)} className="pl-8" />
      </div>
      ...
      {filtered.length === 0 ? (
        <EmptyState
          ...
          action={<Button variant="outline" size="sm" onClick={() => { setRawSearch(""); setSearch(""); setFilters({ account: [], status: [], type: [], bidding: [] }); }}>Clear all filters</Button>}
        />
      ) : (
        <Table>
          <TableHeader>
            <TableRow>
              {(Object.keys(COLUMN_LABELS) as SortColumn[]).map((col) => (
                <TableHead key={col}>
                  <button type="button" onClick={() => handleSort(col)} className="inline-flex items-center gap-1 hover:text-foreground">
                    {COLUMN_LABELS[col]}
                    <SortIcon column={col} />
                  </button>
                </TableHead>
              ...
              <TableRow key={c.id}>
                ...
                <TableCell className="text-right">
                  <div className="flex items-center justify-end gap-1.5">
                    <Button variant="ghost" size="xs" onClick={() => setDetailCampaign(c)}>View</Button>
                    <Button variant="secondary" size="xs" onClick={() => setCloneCampaign(c)}>Clone</Button>
                  </div>
                </TableCell>
              </TableRow>
            ))}
